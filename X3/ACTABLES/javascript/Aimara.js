...
function createTree(p_div,p_backColor,p_contextMenu) {
	var tree = {
		...
		// p_title: Title is used to store additional information on the node. All node attributes are visible when programming events and context menu actions;
		// p_contextmenu: Name of the context menu, which is one of the attributes of the p_contextMenu object created with the tree;
		createNode: function(p_text,p_expanded, p_icon, p_parentNode,p_tag,p_title,p_contextmenu) {
			v_tree = this;
			node = {
				...
				createChildNode: function(p_text,p_expanded,p_icon,p_tag,p_title,p_contextmenu) { return v_tree.createNode(p_text,p_expanded,p_icon,this,p_tag,p_title,p_contextmenu); }
			}
			...
			if (this.rendered) {
				if (p_parentNode==undefined) {
					...
					if (p_parentNode.childNodes.length==0) {
						...
						v_img.src = 'javascript/images/collapse.png';
						v_img.id = 'toggle_off';
						}
						else {
							...
							v_img = p_parentNode.elementLi.getElementsByTagName("img")[0];
							v_img.style.visibility = "visible";
							v_img.src = 'javascript/images/expand.png';
							v_img.id = 'toggle_on';
						}
					}
      ...
      this.adjustLines(document.getElementById(this.name),true);
		...
		drawNode: function(p_ulElement,p_node) {
			...
			if (p_node.childNodes.length == 0) {
				v_exp_col = createImgElement('toggle_off','exp_col','javascript/images/collapse.png');
				...
				if (p_node.expanded) {
					...
					v_exp_col = createImgElement('toggle_on','exp_col','javascript/images/expand.png');
				}
			}
			...
			if (v_icon!=undefined)
				...
				if (p_node.title!=undefined)
					v_a.title=p_node.title;	
				v_a.id = 'a_' + p_node.id;
		...
		expandNode: function(p_node) {
			if (p_node.childNodes.length>0 && p_node.expanded==false) {
				...
				img.id="toggle_off";
				img.src = 'javascript/images/collapse.png';
		...
		collapseNode: function(p_node) {
			if (p_node.childNodes.length>0 && p_node.expanded==true) {
				...
				img.id="toggle_on";
				img.src = 'javascript/images/expand.png';
				elem_ul = img.parentElement.getElementsByTagName("ul")[0];
		...
		toggleNode: function(p_node) {
			if (p_node.childNodes.length>0) {
				...
				if (this.childnodedoubleClickEvent!=undefined)
					this.childnodedoubleClickEvent(p_node);
			}
		},
		...
		removeNode: function(p_node) {
			...
			if (p_node.parent.childNodes.length==0) {
				if (p_node.parent.elementLi!=undefined){
				...
				if (p_node.contextMenu!=undefined) {
					...
					for (var i=0; i<v_menu.elements.length; i++) (function(i){
						...
						if (v_menu.elements[i].submenu!=undefined) {
							...
							v_span_more.appendChild(createImgElement(null,'menu_img','javascript/images/right.png'));
		...
		contextMenuLi : function(p_submenu,p_ul,p_node) {
			...
			for (var i=0; i<p_submenu.elements.length; i++) (function(i){
                ...
                if (p_submenu.elements[i].submenu!=undefined) {					
					...
					v_tree.contextMenuLi(p_submenu.elements[i].submenu,v_ul,p_node);
				}
